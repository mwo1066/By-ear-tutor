...
class ProgressStore:
    ...
    def select_today(self, all_item_names: list[str], today: date, limit: int = 8) -> list[str]:
        """Mix of due reviews (staleest retention first) and new items in roster order."""
        new_items = [n for n in all_item_names if n not in self._states]
        review_items = [n for n in all_item_names if n in self._states]
        review_items.sort(key=lambda n: retention(self._states[n], today))
